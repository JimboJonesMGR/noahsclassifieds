	...
	*/
	...
	function _redirect($filename) {
		...
		*/
		...
		if( !readfile($filename, "r") )
		{
			if( ($ret=file_get_contents($filename))!==FALSE ) echo $ret;
		}
		die();
	}
...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed.= $this->_createGeneratorComment();
		$feed.= $this->_createStylesheetReferences();
		$feed.= "<rss version=\"".$this->RSSVersion."\" xmlns:media=\"http://search.yahoo.com/mrss/\">\n"; 
		$feed.= "    <channel>\n";
		...
		$feed.= $this->_createAdditionalElements($this->additionalElements, "    ", TRUE);
      ...
      {
        $picInfo = $this->items[$i]->image->url;
        $image ="&lt;img src='$picInfo[thumbnail]'; align='left'; height='$picInfo[th_height]'&gt;"; 
    		$feed.= "            <description>".$image.$this->items[$i]->getDescription()."</description>\n";
      }
      else
...
/*			
            ...
            {
                $picInfo = $this->items[$i]->image->url;
                $feed.= "            <media:content medium='image' type='image/$picInfo[ext]' width='$picInfo[width]' height='$picInfo[height]' url='$picInfo[picture]'>\n";
                if( $picInfo['thumbnail'] ) 
                {
                    $feed.= "                <media:thumbnail width='$picInfo[th_width]' height='$picInfo[th_height]' url='$picInfo[thumbnail]'></media:thumbnail>\n";
                }
                $feed.= "            </media:content>\n"; 
            }
*/
  		$feed.= $this->_createAdditionalElements($this->items[$i]->additionalElements, "            ", TRUE);
			$feed.= "        </item>\n";
		}
		$feed.= "    </channel>\n";
		$feed.= "</rss>\n";
