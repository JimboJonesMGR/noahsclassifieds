...
class tar {
		...
		while($main_offset < $tar_length) {
			...
			*/
			...
			if($file_size > 0) {
                ...
                if( !isset($this->numFiles) ) $this->numFiles=0;
				$this->numFiles++;
                ...
                if( !isset($this->numDirectories) ) $this->numDirectories=0;
				$this->numDirectories++;
	...
	function containsFileInDirectory($directory) {
		if($this->numFiles > 0) {
			foreach($this->files as $key => $information) {
				if(strstr($information["name"], "$directory/"))
					return true;
			}
		}
	...
	function removeFilesFromDirectory($dirname) {
        $removedFiles = array();
		if($this->numFiles > 0) {
			foreach($this->files as $key => $information) {
				if(strstr($information["name"], "$dirname/") ) {
                   $removedFiles[]=$information["name"];
					$this->numFiles--;
					unset($this->files[$key]);
				}
			}
