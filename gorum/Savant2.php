...
class Savant2 {
	...
	*/
	...
	function & get($name)
	{
		...
		if( $this!=$gorumview && isset($gorumview->tpl->{$name}) ) return $gorumview->tpl->{$name};
        $ret = "";
		return $ret;
	}
    
    function currentPageIs($queryString)
    {
        global $gorumroll;
        
        $ctrl = new AppController($queryString);
        if( $gorumroll->method!=$ctrl->method || $gorumroll->list!=$ctrl->list ) return FALSE;
        if( $ctrl->rollid==='*' ) return TRUE;
        return ($gorumroll->rollid==$ctrl->rollid); 
    }
    
    function addLangText($label)
    {
        global $lll;
        if( isset($lll[$label]) ) echo $lll[$label];
    }
    ...
    function displayContent($element="", $putWhiteSpace=0)
    {
        ...
        if( !$element ) $view =& View::getContentView(); 
        else $view =& View::getView($element); 
        if( !$view->getEmpty() && 
            ($putWhiteSpace==putWhiteSpaceAbove || $putWhiteSpace==putWhiteSpaceAboveAndBelow) ) echo $contentSeparator;
        $view->displayContent();
        if( !$view->getEmpty() && 
            ($putWhiteSpace==putWhiteSpaceBelow || $putWhiteSpace==putWhiteSpaceAboveAndBelow) ) echo $contentSeparator;
    }
    
    function & applyDisplayLengthLimit( &$s, $displayLength )
    {
        CustomfieldContainer::applyDisplayLengthLimit( $s, $displayLength );
        return $s;
    }
    //Zoltan: end
}
