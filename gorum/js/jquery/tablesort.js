...
var Sort = (function(){
	...
	sort.numeric_converter = function(separator) {
		return function(val) {
			if (typeof(val)=="string") {
				// Zoltan: modositottam ugy hogy a '-' jel ne torlodjon!
	...
	sort.date.formats = [
		// Wed, MM/DD/YYYY HH:MM
		{ re:/\w{3}, (\d{2})\/(\d{2})\/(\d{4}) (\d{2}):(\d{2})/ , f:function(x){ return (new Date(+x[3],+x[1],+x[2],+x[4],+x[5])).getTime(); } }
		// YY[YY]-MM-DD
	...
	table.filter = function(o,filters,args) {
		...
		if (!filters) {
			...
			if (filters.nodeName=="INPUT" && filters.type=="text") {
				...
				filters={ 'filter':"/"+filters.value+"/" }; // Nem csak szo eleji egyezesek!
			}
			...
			for (var i=0,L=filters.length; i<L; i++) {
				...
				if (typeof(filter.filter)=="string") {
					...
					if (filter.filter.match(/^\/(.*)\/$/)) {
						...
						filter.filter = new RegExp(RegExp.$1, 'i');  // case insensitive
